...
const VALID_CATEGORIES = ["women", "men", "accessories", "sale"];
...
function renderProducts() {
  ...
  let filtered = PRODUCTS.filter(p => {
    const matchCat = selectedCats.length === 0 || selectedCats.some(cat => {
      if (cat === "sale") return p.badge === "Sale";
      return p.cat === cat;
    });
...
function catColor(product) {
  ...
  return product.badge === "Sale" ? map.sale : map[product.cat] || "#f0f0f0";
}
